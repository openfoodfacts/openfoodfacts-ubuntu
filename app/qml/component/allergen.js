...
function isContainsAllergen(str) {
    ...
    for (var i=0; i < openFoodFacts.allergenModel.count; i++) {
        ...
        for (var j=0; j < openFoodFacts.settings.allergen.length; j++ ) {
            ...
            // if string match 100%s
            if (lowerAllergenKey.localeCompare(allergenModelKey) == 0 ) {

                // add allergen in native language
                list_allergen = list_allergen.concat(removeDiacritics(openFoodFacts.allergenModel.get(i).products).split(", "));
                // add french allergen
                list_allergen = list_allergen.concat(removeDiacritics(openFoodFacts.allergenModel.get(i).products_fr).split(", "));
            }
        }
    ...
    for(i=0; i < list_allergen.length; i++) {

        if(str.match(list_allergen[i].toLowerCase(), 'gi')){
            return true;
        }
    }
